...
public class SrtParser {
    public static TextTrackImpl parse(InputStream is) throws IOException {
        LineNumberReader r = new LineNumberReader(new InputStreamReader(is, C.UTF8_NAME));
        TextTrackImpl track = new TextTrackImpl();
        while (r.readLine() != null) {
            String timeString = r.readLine();
            String lineString = TtmlNode.ANONYMOUS_REGION_ID;
            while (true) {
                String s = r.readLine();
                if (s == null || s.trim().equals(TtmlNode.ANONYMOUS_REGION_ID)) {
