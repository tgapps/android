...
public class Stripe {
    ...
    public void createToken(@NonNull Card card, @NonNull @Size(min = 1) String publishableKey, @Nullable Executor executor, @NonNull TokenCallback callback) {
        if (card == null) {
            try {
    ...
    public void setDefaultPublishableKey(@NonNull @Size(min = 1) String publishableKey) throws AuthenticationException {
        validateKey(publishableKey);
        this.defaultPublishableKey = publishableKey;
    }

    private void validateKey(@NonNull @Size(min = 1) String publishableKey) throws AuthenticationException {
