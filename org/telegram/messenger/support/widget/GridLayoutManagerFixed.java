        ...
        while (working) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (this.mOrientationHelper.getDecoratedMeasurement(view) != maxSize) {
                    GridLayoutManager.LayoutParams lp = (GridLayoutManager.LayoutParams) view.getLayoutParams();
                    Rect decorInsets = lp.mDecorInsets;
                    measureChildWithDecorationsAndMargin(view, LayoutManager.getChildMeasureSpec(this.mCachedBorders[lp.mSpanSize], 1073741824, ((decorInsets.left + decorInsets.right) + lp.leftMargin) + lp.rightMargin, lp.width, false), MeasureSpec.makeMeasureSpec(maxSize - (((decorInsets.top + decorInsets.bottom) + lp.topMargin) + lp.bottomMargin), 1073741824), true);
                }
            }
