...
public class DefaultDashChunkSource implements DashChunkSource {
    ...
    public void getNextChunk(MediaChunk previous, long playbackPositionUs, long loadPositionUs, ChunkHolder out) {
        ...
        if (this.fatalError == null) {
            ...
            long presentationPositionUs = (C.msToUs(r0.manifest.availabilityStartTimeMs) + C.msToUs(r0.manifest.getPeriod(r0.periodIndex).startMs)) + j2;
            if (r0.playerTrackEmsgHandler == null || !r0.playerTrackEmsgHandler.maybeRefreshManifestBeforeLoadingNextChunk(presentationPositionUs)) {
                ...
                if (availableSegmentCount == -1) {
                    ...
                    long liveEdgeTimeUs = getNowUnixTimeUs() - C.msToUs(r0.manifest.availabilityStartTimeMs);
                    long periodStartUs = C.msToUs(r0.manifest.getPeriod(r0.periodIndex).startMs);
                    firstAvailableSegmentNum = liveEdgeTimeUs - periodStartUs;
                    if (r0.manifest.timeShiftBufferDepthMs != C.TIME_UNSET) {
                        max = Math.max(firstAvailableSegmentNum2, representationHolder.getSegmentNum(firstAvailableSegmentNum - C.msToUs(r0.manifest.timeShiftBufferDepthMs)));
                    } else {
