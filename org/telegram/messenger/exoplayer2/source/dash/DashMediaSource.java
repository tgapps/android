...
public final class DashMediaSource implements MediaSource {
    ...
    private static final class DashTimeline extends Timeline {
        ...
        public Timeline.Period getPeriod(int periodIndex, Timeline.Period period, boolean setIdentifiers) {
            ...
            return period.set(id, num, 0, this.manifest.getPeriodDurationUs(periodIndex), C.msToUs(this.manifest.getPeriod(periodIndex).startMs - this.manifest.getPeriod(0).startMs) - this.offsetInFirstPeriodUs);
        }
    ...
    private DashMediaSource(DashManifest manifest, Uri manifestUri, org.telegram.messenger.exoplayer2.upstream.DataSource.Factory manifestDataSourceFactory, Parser<? extends DashManifest> manifestParser, org.telegram.messenger.exoplayer2.source.dash.DashChunkSource.Factory chunkSourceFactory, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory, int minLoadableRetryCount, long livePresentationDelayMs, Handler eventHandler, MediaSourceEventListener eventListener) {
        ...
        r0.expiredManifestPublishTimeUs = C.TIME_UNSET;
        ...
        r0.refreshManifestRunnable = new Runnable() {
            public void run() {
                DashMediaSource.this.startLoadingManifest();
            }
        };
        r0.simulateManifestRefreshRunnable = new Runnable() {
            public void run() {
    ...
    private void processManifest(boolean scheduleRefresh) {
        ...
        if (!dashMediaSource.manifest.dynamic || lastPeriodSeekInfo.isIndexExplicit) {
            ...
            windowChangingImplicitly = false;
            currentEndTimeUs = Math.min((getNowUnixTimeUs() - C.msToUs(dashMediaSource.manifest.availabilityStartTimeMs)) - C.msToUs(dashMediaSource.manifest.getPeriod(id).startMs), currentEndTimeUs);
            if (dashMediaSource.manifest.timeShiftBufferDepthMs != C.TIME_UNSET) {
                timeShiftBufferDepthUs = C.msToUs(dashMediaSource.manifest.timeShiftBufferDepthMs);
                int periodIndex = id;
        ...
        if (dashMediaSource.manifest.dynamic) {
            ...
            if (presentationDelayForManifestMs == -1) {
                presentationDelayForManifestMs = dashMediaSource.manifest.suggestedPresentationDelayMs != C.TIME_UNSET ? dashMediaSource.manifest.suggestedPresentationDelayMs : 30000;
            }
            timeShiftBufferDepthUs = windowDurationUs - C.msToUs(presentationDelayForManifestMs);
