...
public class PhonebookSearchAdapter extends SelectionAdapter {
    ...
    private void processSearch(final String query) {
        AndroidUtilities.runOnUIThread(new Runnable() {
            public void run() {
                ...
                final ArrayList<Contact> contactsCopy = new ArrayList(ContactsController.getInstance(currentAccount).contactsBook.values());
                final ArrayList<TL_contact> contactsCopy2 = new ArrayList(ContactsController.getInstance(currentAccount).contacts);
                Utilities.searchQueue.postRunnable(new Runnable() {
                    public void run() {
                        String search1 = query.trim().toLowerCase();
                        if (search1.length() == 0) {
                            PhonebookSearchAdapter.this.updateSearchResults(query, new ArrayList(), new ArrayList());
                            return;
                        }
                        int a;
                        ...
                        PhonebookSearchAdapter.this.updateSearchResults(query, resultArray, resultArrayNames);
                    }
                });
            }
        });
    ...
    private void updateSearchResults(final String query, final ArrayList<Object> users, final ArrayList<CharSequence> names) {
        AndroidUtilities.runOnUIThread(new Runnable() {
            public void run() {
                PhonebookSearchAdapter.this.onUpdateSearchResults(query);
                PhonebookSearchAdapter.this.searchResult = users;
                PhonebookSearchAdapter.this.searchResultNames = names;
        ...
        if (holder.getItemViewType() == 0) {
            ...
            if (object instanceof Contact) {
                ...
                if (contact.user != null) {
                    ...
                    userCell.setData(null, (CharSequence) this.searchResultNames.get(position), contact.phones.isEmpty() ? TtmlNode.ANONYMOUS_REGION_ID : PhoneFormat.getInstance().format((String) contact.phones.get(0)), 0);
                }
            } else {
                user = (User) object;
            }
